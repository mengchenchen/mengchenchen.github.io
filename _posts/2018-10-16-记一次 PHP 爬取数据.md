...
function start2($url)
{
   ...
   if (!$str = curl_get_contents($url)) {
      file_put_contents('./txt/errors.txt', $url . ' 这个页面获取失败' . PHP_EOL, FILE_APPEND);
      return;
   }
   $html = str_get_html($str);
   foreach (['citytable', 'countytable', 'towntable'] as $class) {
      switch ($class) {
         case 'citytable':
            $length = 2;
            break;
      ...
      if ($html->find('.' . $class . ' tr td')) {
         $element = $html->find('.' . $class . ' tr td');
      }
   }
   if (!isset($element) || !isset($length)) {
      file_put_contents('./txt/errors.txt', $url . ' 找不到元素类' . PHP_EOL, FILE_APPEND);
      return;
   }
   foreach ($element as $key => $value) {
      $content = $value->find('a', 0) ? $value->find('a', 0)->innertext : $value->innertext;
      if (in_array(iconv('gb2312', 'UTF-8', $content), ['统计用区划代码', '名称']))
         continue;
      $sql = '';
      if ($key % 2 == 0) {
      ...
      echo $url . PHP_EOL;
      file_put_contents('./txt/sql2.txt', $sql, FILE_APPEND);
      if ($value->find('a', 0)) {
         start2($host . $value->href);
      }
   }
}
```
